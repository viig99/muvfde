...
def test_generate_fde_for_query():
    ...
    fde_dimension = num_repetitions * (2**num_simhash_projections) * num_projection_dimensions
    ...
    input_embedding_matrix = np.random.rand(40, 768).astype(np.float32)  # 40 points, 768 dimensions

    fde = muvfde.generate_fixed_dimensional_encoding(input_embedding_matrix, config)
